...
import { verifyEvents, firstValidEvent } from '../src/shared/nostr-events.js';
...
test('larger batches verify fully', async () => {
    const evs = [];
    for (let i = 0; i < 7; i++) evs.push(await signedEvent('batch ' + i));
    const { valid, dropped } = await verifyEvents(evs);
    assert.equal(valid.length, 7);
    assert.equal(dropped, 0);
});

test('firstValidEvent: first signature-valid copy wins, forged copies skipped', async () => {
    const real = await signedEvent('the honest copy');
    const forged = { ...real, content: 'relay-injected lie' };   // same id, bad hash
    assert.equal(await firstValidEvent([forged, real]), real);
    assert.equal(await firstValidEvent([real, forged]), real);
    assert.equal(await firstValidEvent([forged]), null);
    assert.equal(await firstValidEvent([]), null);
    assert.equal(await firstValidEvent(undefined), null);
});
