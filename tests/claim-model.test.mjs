...
// --- Fact layer: RETIRED (2026-07-20) ---------------------------------------
// The Phase 19 fact layer is gone. These pins keep it gone: a supplied
// `fact` field is IGNORED by create (never validated, never stored),
// and parseClaimEvent ignores foreign ['fact', ...] tags instead of
// hydrating them.

test('claim: create IGNORES a supplied fact field — the fact layer is retired', async () => {
    resetState();
    const claim = await ClaimModel.create({
    ...
    assert.ok(!('fact' in claim), 'no fact key on a fresh record');
    const fetched = await ClaimModel.get(claim.id);
    assert.ok(!('fact' in fetched));
});

test('claim: parseClaimEvent ignores legacy/foreign fact tags', () => {
    ...
    assert.ok(!('fact' in parsed), 'foreign fact tags are unknown tags now');
    assert.equal(parsed.text, 'Founded 1948.', 'the claim itself still parses');
});
