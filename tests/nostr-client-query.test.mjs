...
test('a forged frame reusing a real id cannot censor the honest copy', async () => {
    const urlBad = 'wss://fake.relay/five-bad';
    const urlGood = 'wss://fake.relay/five-good';
    const real = await signedEvent('the honest event');
    const forged = { ...real, content: 'forged body under a stolen id' };
    FakeWebSocket.script.set(urlBad, [forged]);    // hostile relay answers first
    FakeWebSocket.script.set(urlGood, [real]);

    const out = await NostrClient.queryRelays([urlBad, urlGood], { kinds: [1] }, 500);
    assert.equal(out.events.length, 1);
    assert.equal(out.events[0].content, 'the honest event');
    assert.equal(out.invalid, 0);   // a valid copy existed → the id isn't counted invalid
});
