...
// line, or EOF — into its month's file (created with the header when
// absent; appended at the bottom; skipped when that heading is already
// there), then regenerates the index from the monthly files. Index
// bullet lines the merge may have swept into a stray's body are
// dropped (they are regenerated anyway). The FIRST run was the
// migration itself: the whole old file was strays. Strays move
// oldest-first (date ascending; within a date, later in the index =
// older, since the index is newest-first), which is what made the
// migration reproduce the oldest-first layout without a hand edit.
//
...
// heading is a well-formed entry heading, its date's month equals the
// file name, and no heading repeats across all files.
//
...
// silent); a stray whose heading already exists in its month is
// skipped even when its body differs (reported on stdout).
//
...
export const ENTRY_HEADING_RE = /^## (\d{4}-\d{2}-\d{2}) — (.+)$/;
...
export function listHeadings(text) {
    const out = [];
    text.split('\n').forEach((line, i) => {
        if (line.startsWith('## ')) out.push({ line: i + 1, text: line, wellFormed: ENTRY_HEADING_RE.test(line) });
    });
    return out;
}

function makeEntry(m, line, lineNo) {
    return { line: lineNo, date: m[1], month: m[1].slice(0, 7), heading: line.slice(3).trim(), title: m[2].trim(), tags: null, text: '' };
}
...
export function parseEntries(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m) { cur = makeEntry(m, lines[i], i + 1); start = i; }
    }
...
// `## ` line, either marker, the GENERATED line, or EOF. `rest` is the
// index text with the stray blocks cut out.
export function extractStrays(indexText) {
    ...
    while (i < lines.length) {
        ...
        let j = i + 1;
        while (j < lines.length && !lines[j].startsWith('## ') && !STOP_LINES.has(lines[j])) j++;
        const body = [lines[i], ...lines.slice(i + 1, j).filter((l) => !INDEX_BULLET_RE.test(l))];
        strays.push(finish(makeEntry(m, lines[i], i + 1), body, 0, body.length));
        i = j;
    }
    return { strays, rest: kept.join('\n') };
}
...
export function collectEntries(files) {
    ...
    for (const { file, text } of files) {
        ...
        for (const h of listHeadings(text)) {
            if (!h.wellFormed) throw new Error(`${file}:${h.line}: not an entry heading (## YYYY-MM-DD — title): ${h.text}`);
        }
...
export function run({ check = false, root = ROOT, log = console.log } = {}) {
    ...
    // (a) move strays, oldest first.
    const { strays } = extractStrays(indexText);
    strays.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : b.line - a.line));
    const pending = new Map();
    for (const s of strays) {
        const rel = monthlyFileFor(s.date), abs = join(root, rel);
        const text = pending.get(rel) ?? (existsSync(abs) ? readFileSync(abs, 'utf8') : monthlyHeader(s.month));
        if (parseEntries(text).some((e) => e.heading === s.heading)) {
            log(`skip  ${INDEX_PATH}:${s.line} — ${rel} already has: ${s.heading}`);
            continue;
        }
        pending.set(rel, endWithOneNewline(text) + '\n' + s.text + '\n');
    ...
    if (!check) {
        ...
        for (const [rel, text] of pending) writeFileSync(join(root, rel), text, 'utf8');
    }

    // (b) regenerate from the monthly files.
    const rendered = renderIndex(collectEntries(readJournalDir(root)));
    if (check) {
    ...
    log(`wrote ${INDEX_PATH} (${rendered.split('\n').length} lines; ${strays.length} stray(s) processed)`);
    return 0;
}
