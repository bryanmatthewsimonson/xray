...
- Process files are `.github/**`, `.claude/skills/**`,
  ...
  `docs/JOURNAL.md` alone never counts as an entry. A cited date must
...
- The labels are case-sensitive literals, matched at line start through
  ...
  quoted commit subject, not the field. Values may combine allowed
  tokens (`unit + guard`) and carry prose after them. HTML comments and
  fenced code are stripped first. Default: keep.
...
`docs/journal/2026-08.md`. That is expected: no body predates the
contract.

**Verified by** `tests/pr-body-check.test.mjs` (22 tests: every rule
both ways, the template blank (fails) and filled (passes), comment
and fence stripping, the Dependabot skip, the annotation escaping, the
workflow's triggers and permissions, and every `src/` file in exactly
one lane or in UNASSIGNED). Twenty-eight negative controls, each a
one-line mutation of the checker, the lanes table, the template or the
workflow, all turned the suite red.
