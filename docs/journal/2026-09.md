...
failed read rejects and caches nothing. Plain reads keep the old
default-view fallback, uncached, so no surface breaks. Writes,
`getStrict` and `activeWorkspaceId({ strict: true })` fail closed, and
global keys no longer consult the pointer. The backup restore and merge
...
written (…)" instead of "not found" or success. The side panel's
legacy array import rethrows that error instead of skipping the row.

Tests: `tests/entity-registry-store.test.mjs` has 31 cases, with two
module instances standing in for two pages over one fault-injecting
stub. 30 were red on origin/main; the concurrent-create case is a green
control, because creates already serialize through the keystore lock.
In a real browser (the built extension, headless), origin/main passed
4 of 17 checks and this branch 17 of 17. On origin/main, 20 of 40
concurrent updates from two pages survived in every round, and the
real side panel's Import (a case bundle, and a legacy entity array)
erased all 4 seeded records behind a success toast.

Not fixed (follow-ups): the other registries (claims, assessments,
platform accounts, …) still read-modify-write with the fail-open
`Storage.get`. `setActiveWorkspaceId` caches a pointer whose write
failed. IndexedDB names (`workspaceDbName`, `resolveActiveDbName`)
still fall back to the default workspace on a failed pointer read.
