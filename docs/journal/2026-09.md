...
written (…)" instead of "not found" or success. Every store-level
refusal is a typed `StoreRefusedError`; the side panel's array import
and the case-bundle import stop on it by name, and a bundle import over
...
Tests: `tests/entity-registry-store.test.mjs` has 41 cases, with two
module instances standing in for two pages over one fault-injecting
stub. 36 are red on origin/main. Five are green there: S1–S3 (the
...
unstamped-pull case (it pins a first-cut regression). S1–S3 and the
strict-writer-under-a-fallen-back-cache case are red on the
read-nothing cut. Five negative controls each turn their cases red:
the locked write mapped through the page's cache, the strict pointer
read served from the cache, the removal's active-workspace check from
the cache, the reset's deletes through the cache, and an uncached plain
fallback. In a real browser (the built extension, headless), the
...
read-back through the plain cache (`EntityModel.create`'s return, the
entity list) shows the default workspace's view. Two plain operations
...
nothing writes it back, and a read and the write built on it must
resolve the workspace the same way.**
