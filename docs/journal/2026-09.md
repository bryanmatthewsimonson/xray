...
Wire: no tag or kind changes, but values change. A Reels-viewer capture
used to publish `r` (and `d` = its hash) as whatever the head named:
the slashless `…/reel/<code>`, or another account's grid. It now
publishes `https://www.instagram.com/reel/<code>/`, the address a
`/reel/` capture of the same reel already uses, so a re-capture gets a
new `d`. A stale-head capture also drops the other page's `image` tag
(og:image became `featuredImage`, event-builder.js:219) and the foreign
author, caption and counts from its body. Already-published events stay
as they are (#368's disposition).
