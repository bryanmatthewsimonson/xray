...
annex and guard. Its own principle 6 forbids that ("the corpus is
cited, never mirrored"), and the skills README names the pattern as
the doc-drift class. RESET_PLAN §9 said to fold it. The fold is a
119-line skill. Kept: the eight review standards, the invoke list,
the protocol and report sections, the failure-mode pair, the
boundaries and seams, one codification candidate, the §0 question,
and the directive quote. Dropped as mirror: the corpus map, and first
principles 1–4, which restated CONSTITUTION Art. 1 and Art. 2, the
citation convention, Art. 13 and Art. 12. Each standard now cites
the article it applies. Also dropped: the release-tag trigger (the
preflight ordering is "all eight") and the supersession-log guard
candidate. The README keeps #365's roster row, five routing rows and
seam bullet. architect keeps #365's two seam pointers: governance
writes the bug-or-unratified-amendment finding on a red normative
guard, and architect gives the structural verdict. "Ruling" became
...
for the README seam bullet and the architect edits. CLAUDE.md is not
touched here; its skills sentence is a sibling PR's.
