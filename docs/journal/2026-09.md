...
an id, then `llmJobRequestHash(request)` — SHA-256 of the exact request
— so a kept result is only reused for the byte-identical request it
answered (the id: the case; case + subject for the forensic pass;
`registry` for the entity audit; the audited slice's hash for the
Quick audit). The decisions worth second-guessing:
...
- **Extracted, not raised.** The reader sat exactly at its 8297-line
  ...
  (8281 after; the 330 s page-side race is gone — the pass's own abort
...
worker's `fetch`, egress killed; 43/43 checks) caught what every unit
