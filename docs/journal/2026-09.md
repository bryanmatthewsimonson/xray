...
`mergePulledRows`. For the pointer, an absent value is 'default'; a
failed read rejects and caches nothing. Writes, `getStrict` and
`activeWorkspaceId({ strict: true })` fail closed, and global keys no
longer consult the pointer. A plain `Storage.get` of a content key
under a failed pointer read reads NOTHING (the caller's default, as for
a failed content read) and marks the key; a plain `set`/`delete` of a
marked key refuses until a plain read of it succeeds. Writes built on a
strict read (the locked read-modify-write, the reset's deletes) pass
`strictRead` and are not blocked. The backup restore and merge
read the pointer and the whole storage area strictly: a failed read had
made a merge write the file's registry over the local one. The safety
backup taken before a reset now reads strictly too. `resetWorkspace`
reads the pointer first and stops when a delete fails. The restore,
merge, reset and workspace removal take the registry lock, nested
inside the keystore lock where both apply; nothing takes the two locks
in the other order.
...
Hardening, same day, after review. The first cut kept plain reads on
the default view, uncached, while writes re-read the pointer. Two
verifiers showed that split was worse than main: one failed pointer
read in `ClaimModel.create` (any plain `get` → `set`) read the DEFAULT
workspace's claims and wrote them, plus the new one, into the ACTIVE
workspace, erasing its claims. origin/main had only misrouted the new
claim into the default. The read-nothing rule above replaced it. Same
...
stub. 39 are red on origin/main (the concurrent-create case is a green
control; the unstamped-pull case pins a first-cut regression) and 10
on the first cut; eleven negative controls each turn their case red.
In a real browser (the built extension, headless), origin/main passed
7 of 26 checks, the first cut 19 and this branch 26. On origin/main,
20 of 40 concurrent updates from two pages survived in every round,
and the side panel's Import erased all 4 seeded records behind a
success toast; on the first cut the ClaimModel straddle left the
active workspace holding the default's claim and not its own two.
...
and a refused write returns false that those models ignore (the error
is printed, the UI may still say success). The refusal is per key and
cleared by any successful plain read of that key, so another caller
reading it between a fallen-back read and its write reopens the gap,
inside that same race. If the pull's record phase fails after its keys
were installed, a local record older than the pulled one keeps its
stale fields while its key slot holds the pulled key, until the next
pull. `setActiveWorkspaceId` caches a pointer whose write failed.
