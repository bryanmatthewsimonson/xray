...
Already-published events whose `d` came from a stale og:url are not
superseded by this fix: a re-capture publishes under the constructed
address as a new `d`, and the old event stays where it is. That is the
right outcome — correcting a signed public record is a deliberate act,
not a side effect of a capture fix (RESET_PLAN §9's disposition for
#368, recorded 2026-09-25 when the branch was brought onto the split
JOURNAL).
