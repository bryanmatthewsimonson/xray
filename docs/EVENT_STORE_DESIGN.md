# The local event store — store-first publish (Phase 29, proposed)

> **Status:** design for maintainer review (2026-08-01). Commissioned
> from planning (maintainer: "X-Ray the client needs to be thoughtfully
> designed"). Nothing here is built. The phase number is proposed, not
> settled (Review Q1). Review questions at §13; slices at §11. The
> NIP-01 filter engine is deliberately **not** designed here — see
> §4.3.
...
Rejected for v1. The transcriber companion's loopback pinning
(127.0.0.1/localhost/[::1] only) is precedent if a dev-tooling tier is
ever wanted — deferred, own decision (Review Q6).
...
- `held` — excluded from flushing by policy (mechanism only; whether
  anything ever sets it is Review Q3).
...
- **Foreign pending rows JOIN the local queue** (proposed — Review
  Q7). Re-publishing is idempotent by construction (immutable,
  id-keyed events; `src/shared/confirmed-publish.js`), so the worst
  case of both machines flushing the same row is a relay no-op. The
  flusher's pacing and backoff absorb the rate-limit exposure (§6).
...
- **The late-arrival question** (the real coordination item): the
  ...
  though the relay accepted it. `PLAN.md` is silent on this. Raise it
  on the crux side (drop `since` from the live filter and rely on
  idempotent writes, or periodic re-scan) **before** X-Ray ships a
  long-lived queue (Review Q4).
...
- **`is_private`**: `PLAN.md`'s model is a server-filtered flag —
  ...
  flushed" would be STRONGER than the plan and would break the /me
  feed and the export-bundle expectations. §3.1's `held` state is the
  mechanism only; adopting a local-only privacy tier is Review Q3.
...
every flag since Phase 9a defaults false; flip-on is a
post-smoke-walk decision, Review Q2). `storeFirstPublish` falls
...
No localhost or native relay process (the JOURNAL 2026-07-08
contingency stands documented and untriggered; loopback dev tooling
is a separate later decision, Review Q6); no live relay subscriptions
...
| 29.2 | The flusher: `alarms` lifecycle + backoff, OK-prefix classification, computed supersession, cross-workspace queue + deferred ledger marks, portal pending-flush view, delete/reset warnings, summary honesty line, shared pacing constant | §3.3–3.4, §7 |
...
## §13. Review questions (for the maintainer)

1. **Phase number**: Phase 29, or an unnumbered wave in the post-28
   style? (Proposed: a numbered phase — this sets posture other docs
   will cite. The H1 stays "proposed" until ruled.)
2. **Flag flips** (§9): both flags ship off; what gates flipping
   `storeFirstPublish` on — the 29.1/29.2 smoke rows alone, or a
   soak period? And may 29.1's sign-time journaling ship unflagged
   (it is strictly additive durability; the ledger-marking rule is
   unchanged)?
3. **`is_private`**: adopt a local-only `held` tier (stronger than
   crux `PLAN.md`'s server-filtered model, and divergent from it), or
   follow the plan and leave `held` unused?
4. **The crux indexer late-arrival question** (§6): raise it as a
   crux.immo issue now, or gate the long-lived queue (29.2) on its
   resolution?
5. **Cache subsumption order**: network first, then portal (proposed
   — network-cache is the younger and simpler consumer), or leave
   both and let the store serve only new consumers?
6. **Localhost relay dev tier**: JOURNAL 2026-07-08 already keeps a
   self-hosted relay as a documented contingency with a named
   trigger. Record a LOCALHOST dev tier as separate sanctioned
   tooling under the transcriber-companion loopback precedent, fold
   it into that contingency, or leave both untouched?
7. **Merge-imported pending rows** (§3.4): join the local flush queue
   as proposed (idempotent-safe, rate-limit exposure absorbed by
   backoff), or arrive `held` for manual release?
