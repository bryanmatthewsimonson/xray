...
researchers, reads their published artifacts, and pulls the convincing
pieces into their own corpus one reviewed accept at a time. Implements
the queued **KS.5–KS.8** slices (the KS section above;
`KNOWLEDGE_SHARING_DESIGN.md` amended 2026-07-16 — global follow
scope, opt-in kind-3 mirror) on a new standalone **Network page**
(`docs/NETWORK_CLIENT_DESIGN.md`, normative for the surface).
Maintainer decisions: v1 = follows + feed + incorporation — **no
kind-1 notes, no DMs** ("conversation" is the existing judgment
...
Non-goals (v1): no kind-1 notes/replies; no DMs; no live relay
