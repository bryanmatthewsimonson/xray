...
| 2026-09-24 | **Corpus reduce as a JOB — rows 20.m + 20.n** (PR #374; agent walk, headless Chromium 141, fresh profile, `api.anthropic.com` stubbed in-process with a valid SSE tool-use stream, a dead proxy as the backstop — no spend, no traffic left the machine; an independent verifier re-ran the script, 18/18 checks) | **PASS.** 20.m: reload 15 s into a 60 s reduce, reopen the case → "A synthesis for this corpus is still running in the background — Analyze corpus… attaches to it"; the confirm ended "(no new synthesis call)"; **one** reduce request in total; the brief rendered; the `xray:llm-job:default:corpus-reduce:…` key was acked (gone) after the brief rendered. 20.n: worker stopped 15 s into a 75 s reduce (CDP `ServiceWorker.stopWorker`, the internals page's Stop) → a new worker instance woke on the next poll and ~2 s later the status read "Synthesis failed: the service worker restarted during the call, so its result was never produced. Retry re-runs only the synthesis stage — the per-article extracts are cached and free — but that synthesis call is billed again."; the old "retry cheap" copy never appeared; the retry made map 0, reduce 1. The in-worker heartbeat was seen rewriting the record ~every 20 s. **Limit, stated plainly:** Playwright's attached debugger switches off MV3 idle termination, so this walk proves reattach/dedupe across a reload and the honest failure after a kill — it cannot show whether a long REAL reduce now completes without DevTools attached; the maintainer's next large Analyze is that observation. Three small notes from the walk fixed on the PR (previous-brief sentence on failure, elapsed counter anchored to the record, the 20.m key shape). |
| 2026-09-24 | **PDF link annotations — real-world PDF** (PR #391 soak walk) | **PASS (Chrome); Firefox not walked.** The maintainer, on the branch build, captured a web PDF that links to other sites from the toolbar. They reported the check as passed against its Expect line: 🎙 Media… → "Outbound links — how they're cited (N)" appeared, each row was labelled with the words the link sits on in the PDF, and hovering a row showed its URL. Which PDF and how many links were not reported. This is the first real-world observation of the Phase 27 PDF-link path, which had never produced a link since pdf.js 6 (JOURNAL 2026-09-24). Those row labels are the anchor text each `link` tag publishes. The optional Firefox step was skipped; the change uses no browser-specific API. |
| 2026-09-24 | **Option C — the named refusal, and legacy entities after the upgrade** (PR #324; **AGENT walk, headless** Chromium `--headless=new`, a brand-new profile per case — not a human soak row; outcomes from the PR's 2026-09-24 comment) | **PASS (agent).** In the reader, an article was captured, a name drag-selected, and the tagger's "New as: person" clicked. On the PR build with no local primary — under default signing and under NIP-07 — the popover read "Create failed: Entity identities need a local primary identity to derive from — create or import one under Settings → Signing. …" and no entity, key or tag was written. With a primary seeded from fixture key TV1, the entity was created with `derived_from` = TV1's pubkey. `main` before the PR silently created one with a random key (`derived_from: null`), the behaviour the PR removes. An independent verifier re-ran these and seven more cases on head `1bdb167`, including explicit Local and NIP-07 with a primary: 11/11 matched, no page or console errors. The legacy half ran in node over the real modules with a `chrome.storage` stub, not in a browser: an entity `main` made with no primary kept its pubkey after the upgrade, `restoreDerivedKeys()` left it alone, and `LocalKeyManager.signEvent` gave a valid BIP-340 signature (a tampered copy failed). This covers the creation half of the walk NIP07_IDENTITY_KICKOFF §6 owes ("entity creation under Local and under NIP-07 (expect the refusal without a local primary, derived creation with one)"), at one door, the entity tagger. **Not on the record:** §6's "one entity-tagged publish under NIP-07", and the other creation doors §6 names — "the claim modal's picker", "the sidepanel import", `createCase`, and "the LLM review's accept-all". |
| 2026-09-24 | **pdf.js 6.2.108 → 6.3.289 — PDF capture, PR build against `main`** (PR #387; **AGENT walk, headless** Chromium 141 `--headless=new`, three locally served PDFs opened via `?pdf=<url>`, a fresh profile per build — in place of the human PDF row RESET_PLAN §9.1 set for #376; outcomes from the PR's 2026-09-24 comment) | **PASS (agent), confirmed by an independent re-run.** The reader's `extraction.method` named the engine that ran (`pdfjs-6.3.289` vs `pdfjs-6.2.108`). Text + images (core fonts, a JPEG photo, a PNG chart): markdown byte-identical across builds, two figures each, identical figure pixel hashes. CJK (non-embedded fonts that need `dist/cmaps/`): all six `.bcmap` loads OK, full CJK text, markdown byte-identical. JPX + JBIG2 (need `dist/wasm/`): all four decoder assets loaded, both figures decoded, markdown byte-identical. No page errors and no failed pdf.js asset requests; the runtime assets are byte-identical between builds apart from two LICENSE files. Found on the way, present on `main` too: PDF links were dropped on every capture (fixed by #391; its row is above), and the extension's CSP blocks pdf.js's `.wasm` decoders, so it uses its JavaScript decoders (slower, still correct). |
| 2026-09-24 | **Key store — extension pages stop erasing each other's keys** (PR #392; **AGENT checks, headless** Chrome with the real extension — not a human soak row; outcomes as reported in the PR body, which does not date each check) | **PASS (agent).** Three reader tabs, the side panel and the Settings page were all loaded before any write, then each wrote keys, including the side panel's identity save and forget and Settings' restore. Concurrent creates from two pages kept 12 of 12 keys. A workspace switch made in one page while another stayed open: the open page followed it, and its write landed only in the new workspace. A replace-all backup restore raced a key write in 7 interleavings: "It was never undone, and no key came back." The original repro script fails 20 of its 41 checks on `main` and passes 41/41 on the PR. Firefox not run. |
| 2026-09-25 | **Instagram URL identity** (PR #368 soak walk, maintainer, branch build) | **PARTIAL: 1 PASS, 2 FAIL.** (1) A reel opened from a profile's Reels grid: PASS (verdict as reported; no details given). (2) A reel reached by in-app navigation in the Reels viewer (`/reels/<code>/`) captured with title `Jeff Dye on Instagram: "No one cares about self driving cars"`, AUTHOR blank, no Instagram chips or profile block, PUBLISHED blank, and URL `https://www.instagram.com/reel/DcwbjmXy0B5` (no trailing slash). That is the generic extractor: the URL grammar did not know `/reels/` (JOURNAL 2026-09-25). Fixed on the branch. An agent replay (headless Chromium, fake instagram.com) shows the Instagram path, the constructed URL and the author. **Re-walked 2026-09-25 on the fixed branch (`b1520d1`): PASS.** The maintainer reports that every retest step passed: scrolling two reels on in the Reels viewer (`/reels/<code>/`), the grid regression, and a direct `/reels/<code>/` load (verdict as reported; no details given). |
| 2026-09-25 | **The five remaining single-call LLM passes as JOBS — rows LJ.a–LJ.d** (agent walk on the branch, headless Chromium 141, fresh profile, `api.anthropic.com` answered by a canned SSE tool-use stream patched into the worker's `fetch`, the smoke launcher's dead proxy as the backstop — no spend, no traffic left the machine; 43/43 checks, then 53/53 on the re-walk after the security review) | **PASS after one fix the walk forced.** LJ.a: a reader reload 12 s into a 45 s Quick audit; the second click reattached, the button read "⏳ Auditing… 27s" (the record's clock — a page clock would have read ~15 s), the audit imported ("8 modules valid, 1 prediction"), ONE call in total, and the `xray:llm-job:default:audit-run:…` key was gone after the import. LJ.b: a CDP `ServiceWorker.stopWorker` mid-call gave "Audit failed: the service worker restarted during the call, so its result was never produced. Quick audit makes a new call, billed again." — still on the audit status line after the 7 s toast was gone — and the same honest copy, naming Suggest edges (LLM)…, on the portal's callback + `lastError` transport. LJ.c: a portal reload mid–Suggest links left the finished run on disk un-acked; the next Suggest links rendered it with no second call and the key was acked after the save. Suggest edges across a round trip to the library: kept, then shown free on return — **this is the check that failed first**: the case view is only HIDDEN behind the library, so the first cut (release on `isConnected`) acked a result nobody could see and the next Suggest paid again; fixed as `isShown` (`portal/dom.js`) with a unit test that fails on the old rule. LJ.d: a side-panel audit that finished while another view was up waited on disk and rendered on return, one call. Forensic Analyze subject: rendered and acked through the real transport. **Re-walk (the worker's door):** X-Ray's real content script, in a web page served on loopback, sent all four job ops through its own isolated world (CDP `Runtime.evaluate`) and each came back "LLM jobs answer extension pages only" — no model call, no job record written or removed; the same `find` from the portal was answered by the runner, and a portal start presenting another request's scope key was refused at the worker ("LLM job scope key does not match its request"); rows A–G passed again unchanged. **Limit:** the attached debugger keeps the worker alive, so LJ.e (a real call past 5 minutes, no DevTools) is unobserved |
| 2026-09-25 | **LLM jobs — hand check 1, a real Quick audit in Chrome** (maintainer, branch build, real key) | **PASS on the rerun** (the maintainer's verdict; no further detail given). First attempt: "Audit failed: Unknown LLM job pass: audit-run." on the audit status line and as a toast — not a valid run: the reader page was the branch build while the service worker still ran `main`'s (that string exists only in the worker's job runner, and `main`'s allowlist holds three passes). Reloading the extension card and re-capturing the article cleared it, which confirms that diagnosis: an unpacked extension's pages load from disk on open, but its worker can keep running the build it was registered with until the card is reloaded. Checks 2 (the same Quick audit in Firefox) and 3 (a pass running past ~5 minutes with no DevTools attached): **SKIPPED by the maintainer** (2026-09-26) — unobserved, not passed; both are in the "Not yet walked" note below. |
...
sub-line wording — the section is otherwise walked; and **LJ.e**, any
LLM job's REAL call running past 5 minutes with no DevTools attached
(20.m/20.n and LJ.a–LJ.d were agent-walked, but a debugger keeps the
worker alive — only a real long run observes Chrome's teardown); and any
LLM job in **Firefox** since the 2026-09-25 sender check (the job ops
answer only a sender on `runtime.getURL('')`'s origin — Chrome-walked,
never run on `moz-extension://`).
...
**2026-07-20: the fact layer (typed fields, Add-fact, fact conflicts,
the kind-30067 fact sheet) is RETIRED — the old fact walks are gone.**
What remains testable: the dossier (claims/identity/judgments), entity
health, case scope, and kind-0 + mention-note publishing.

| # | Test | Pass criteria |
|---|---|---|
| 19.c | Side panel → a person entity → **Dossier** block | ✅ the block shows coverage ("N claims · M articles captured"); **Open full dossier** opens the portal dossier view via `#dossier=` deep-link |
| 19.d | Portal dossier | ✅ identity (family/aliases/accounts), captured content rows, judgment distributions (never a score), and co-tagged relationships render; every judgment count routes to its own surface |
| 19.f | Side panel footer → **Health** | ✅ the duplicate report lists suspect pairs with evidence; **Merge…** offers a keep-which choice and links the alias (dossiers re-unify — re-open the dossier and both families' claims aggregate); **Not duplicates** persists across rescans; **Unlink** undoes a merge |
...
| 19.i | **Flag OFF** (default): publish an article with tagged entities; watch the network panel / relay log | ✅ **no entity kind-0 or kind-1 mention note leaves the device** — the publish summary shows no corpus segment |
| 19.j | Options → Entity corpus → ON (read the disclosure) → publish an article with tagged entities | ✅ each tagged entity's kind-0 emits (maintainer line + type + aliases — no biography lines) plus one kind-1 mention note per entity, entity-signed |
| 19.k | Publish the same article again with nothing changed | ✅ no corpus events re-emit (the hash gate skips); rename the entity + publish → the kind-0 re-emits; portal dossier's **Republish profile** force-publishes the kind-0 |
...
| 20.m | **Interrupted synthesis (jobs, 2026-09-05)** — Analyze corpus… on a case whose extracts are all cached; while the status reads "Synthesizing N extracts… Ns — running in the background", **reload the portal tab** and reopen the case | ✅ the block's status says **"A synthesis for this corpus is still running in the background — Analyze corpus… attaches to it"** (or, if it finished during the reload, **"A finished synthesis from an interrupted run is waiting"**); clicking Analyze corpus… shows a confirm ending **"(no new synthesis call)"**, and the brief renders without a second reduce request in the Anthropic console. `chrome.storage.local` shows an `xray:llm-job:default:corpus-reduce:…` key (the segment after the prefix is the workspace — `default` for the default workspace) while the brief is pending and none after the brief renders (the ack) |
| 20.n | The same, but kill the worker mid-call: while "Synthesizing…" shows, `chrome://serviceworker-internals` → Stop the X-Ray worker | ✅ the status reads **"Synthesis failed: the service worker restarted during the call… Retry re-runs only the synthesis stage — the per-article extracts are cached and free — but that synthesis call is billed again."** — never the old "the cached extracts make the retry cheap"; a second Analyze corpus… re-runs ONLY the reduce (the map plan reports every member cached) |
...
## LLM jobs — the remaining single-call passes (2026-09-25)

Suggest links, Suggest edges, the forensic Analyze subject, the side
panel's Audit with LLM…, and the reader's Quick audit ride the job
runner since 2026-09-25 (JOURNAL 2026-09-05, its 2026-09-25 addendum),
like the corpus reduce in 20.m/20.n. Needs `llmAssist` + a key (Suggest
links and Suggest edges also `caseSynthesis`); every row costs real API
calls.
Unit tests drive each consumer through the real runner; what they
cannot stage is a REAL browser dropping a real worker.

| # | Test | Pass criteria |
|---|---|---|
| LJ.a | Reader → a long capture → **Quick audit**; while the button reads "⏳ Auditing… Ns", **reload the reader tab**, then click Quick audit again on the same text | ✅ the button's counter resumes near where it was (never back to 0s); the audit imports ("Audit complete…" toast); the Anthropic console shows ONE audit request in total; `chrome.storage.local` holds an `xray:llm-job:default:audit-run:…` key while it is pending and none after the import |
| LJ.b | Any one of the five while it runs (Quick audit is the cheapest): `chrome://serviceworker-internals` → Stop the X-Ray worker | ✅ the failure reads **"…the service worker restarted during the call, so its result was never produced. <control> makes a new call, billed again."**, naming the control you used (Quick audit / Suggest links… / Suggest edges (LLM)… / Analyze subject… / Audit with LLM…), and it stays readable after any toast is gone (the reader keeps it on the audit status line); nothing retries on its own |
| LJ.c | Portal case → **Suggest links…** (or **Suggest edges (LLM)…**, or the forensic **Analyze subject…**) → confirm → **reload the portal tab** while it runs; reopen the case after the call would have finished and click the same control with nothing changed | ✅ the proposals render with **no new call** (one request in total); the `xray:llm-job:default:corpus-links:…` (resp. `hypothesis-edges` / `forensic-corpus`) key is gone once they are on screen. (Suggest edges / Analyze subject behave the same if you navigate to another portal view instead of reloading; Suggest links saves its run as it lands, so after navigating back its proposals are simply there) |
| LJ.d | Side panel → **Health** → **🤖 Audit with LLM…**; while "Auditing the registry… Ns" shows, switch to another side-panel view; come back to Health and run it again | ✅ the review renders with no new call (if the call was still running when you came back, the counter reappears and the review lands without a click); a FAILED run leaves its reason under the button, not a 3-second toast |
| LJ.e | Whenever one of the five runs past ~5 minutes for real (a large case's claims index, a big registry) with **no DevTools attached** | ✅ the result arrives. A "message channel closed" error means Chrome still tore the worker down under a job — the limit the 2026-09-24 walk could not observe |
