...
| 2026-09-25 | **LLM jobs — hand check 1, a real Quick audit in Chrome** (maintainer, branch build, real key) | **NOT A VALID RUN — rerun pending.** Observed: "Audit failed: Unknown LLM job pass: audit-run." on the audit status line and as a toast. Diagnosis (unconfirmed until the rerun): the reader page was the branch build but the service worker was still running `main`'s — that string exists only in the worker's job runner, and `main`'s allowlist holds three passes (`corpus-map`, `corpus-reduce`, `entity-page`), so a page and worker from the same build cannot produce it. An unpacked extension's pages load from disk on open; its worker can keep running the build it was registered with until the extension card is reloaded. |
