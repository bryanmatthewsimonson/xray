...
| 2026-08-23 | **Portal UX PR-2 — string truth** (PR #349 soak walk) | **PARTIAL.** Confirmed from the maintainer's screenshots: header reads "X-Ray — Archive"; identity chips read "your signer" / "seen in your published events" (raw tokens gone); "Unpublished local items (86)"; "key ownership proof" badge; and C1's vocabulary deliberately untouched ("Local ledger says 68 published", "remote-only"). Empty-state check SKIPPED — the archive resolves normally, not worth breaking the setup. Inspector "View published copy (read-only)" NOT YET OBSERVED — the walk said "open any row's inspector" without a click path, which is itself the finding. The same screenshots surfaced the signing-banner defect fixed in this PR (Settings claimed "not configured yet" on an archive with 68 published events). |
