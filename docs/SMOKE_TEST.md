...
| 2026-08-30 | Margin S1 (M.1–M.14) | pending |
...
| M.3 | Collapsed click on a tinted span, including one covered by 2+ notes of DIFFERENT families | ✅ the side panel focuses that span's card — and on an overlap, ALL covering cards surface together (a claim and the forensic finding about it, not just the first); no authoring popover |
...
| M.6 | Switch to Reader, edit a grounded sentence, switch back to Annotated | ✅ the orphaned note sits in Page notes reading "No longer matches the text after your edit" (the edit is blamed, not the source); that card offers NO "Show in text"; nothing silently vanishes |
...
| M.10 | Family chip toggle (e.g. Claim proposals off), then Accept a proposal from another family | ✅ chip shows struck-through state; that family's cards, tints AND rail markers all hide together; nothing else changes; the toggle SURVIVES the re-render an Accept triggers; toggling back restores |
| M.11 | Flag ON: open an archived PDF with figures, and a diarized transcript, in Annotated | ✅ figures render (no broken images); speaker labels show; tints/cards behave as in M.2 |
| M.12 | Article with audit evidence only | ✅ audited passages show NO tint; hollow-square rail markers are the only indication |
| M.13 | In Annotated: edit body in Reader and return; attach a transcript | ✅ margin's proposal/audit families match the bars beneath (no stale hash families) |
| M.14 | Flag ON: fresh capture | ✅ lands in the editable Reader, not Annotated |
