...
| 2026-08-25 | **Suggest shape-repair round + dossier bandText fix** (PR #358 soak walk) | **PASS by field evidence (retry half); the bandText half is guard-tested, render unobserved.** On this branch's build, the morning's six consecutive shape failures gave way to three completed Suggest runs — 72, 60 and 86 proposals on a long diarized transcript (screenshots on record) — the exact workload that had been failing. The dossier "Captured content" date rendering could not be walked (the maintainer could not find a dossier — itself recorded as a discoverability finding; path: case dashboard → People & organizations → chip → dossier →); its fix is pinned by tests/portal-case-people.test.mjs (no bandText call; bandISO renders the band). |
