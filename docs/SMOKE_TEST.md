...
| 15.17a | On an evidence row: pick a claim from **link claim —** and, on a second row, type a source URL; leave a third row quote-only | ✅ the linked row's quote auto-fills from the claim; the quote-only row shows ⚠ ("carries no source reference on the wire"); all three save |
| 15.17b | Publish (flag on), then inspect the raw 30063 in the portal | ✅ the linked row's `evidence-for` tag carries the claim's 30040 coordinate in the coord slot (if the claim is published — otherwise empty until next publish); the URL row's tag carries the url; the inspector renders the url as a link and the coordinate as a copyable chip |
