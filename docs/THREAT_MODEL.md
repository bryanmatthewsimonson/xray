...
nor Chromium itself can reach a relay or anything else; the MA.6 walk
...
it did not take; the identity lives in a throwaway profile that
`close()` deletes and is never exported; seeds are fiction, never
derived from a real backup, bundle or capture, and no scenario
screenshots a surface after revealing a key; the runner fails the run
when any output file matches an `nsec1…` string or a private-key field
name (`tools/smoke/run.mjs`); the job holds the workflow's read-only
