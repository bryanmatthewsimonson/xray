...
| 2026-09-25 | Entity-registry write path (JOURNAL 2026-09-25). No new boundary, message or destination. **B6**'s control is tightened: every `entities` write — bundle import, sync pull, backup restore / merge, and every local edit — is a read-modify-write of a fresh STRICT read under the Web Lock `xray.entities`, with the keystore's extension-origin refusal. A failed registry or storage-area read now writes nothing; it used to be read as an empty registry, erasing every other record and letting a staler pulled key replace a local one. The workspace pointer: ONE per page. Every read and write — the keystore Map and `signEvent`, the entity list, the IndexedDB names included — resolves the workspace through `storage.js`'s cached pointer, which reads a failed pointer read as the default workspace (origin/main's rule) and caches it until the pointer next changes. Under that fault a page's writes are misrouted consistently into the default workspace, as before (an open follow-up), but nothing read in one workspace is written into or joined with another. So no display, kind-0 or mention signature, entity-sync push or case bundle (which ships private keys) pairs one workspace's record with another workspace's key. The destructive wholesale operations (reset, workspace removal, backup restore / merge / export, the reset's safety backup) first re-read the pointer strictly and refuse, nothing written or downloaded, when it is unreadable or differs from the page's cache. Before this, a page whose cache had fallen back reset (cleared) or replaced the DEFAULT workspace. The reset also refuses any workspace but the one its safety file names. Backup restore, reset and removal nest the registry lock inside the keystore lock; the merge takes only the registry lock; nothing takes them in the other order. `entities` shape unchanged; the reset's safety file gains a `workspace` field (nothing reads that file back); no wire-format change. Guard: `tests/entity-registry-store.test.mjs`. |
