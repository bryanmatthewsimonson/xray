...
| 2026-09-25 | LLM jobs, the remaining single-call passes (JOURNAL 2026-09-05 and its 2026-09-25 addendum). **B4**: five message types REMOVED — `xray:llm:hypothesis-edges`, `xray:llm:corpus-links`, `xray:llm:forensic-corpus`, `xray:llm:entity-audit`, `xray:audit:run` — each a held-open message MV3 could kill with its paid result. Nothing ADDED: the five passes ride the existing `xray:llm:job:{start,status,find,ack}`, validated at `shared/llm-jobs.js` on receipt exactly as the 2026-09-05 row records; the frozen pass allowlist grows from three names to eight (still fail-closed; a guard now pins the worker's pass table set-equal to it, so a pass cannot be reachable without being allowlisted or allowlisted without a function). Same senders (extension pages only — portal, side panel, reader; no `externally_connectable`), same gates inside each pass (flags + key), same page-side firewalls (`validateHypothesisEdges` → grounding → filter; `prepareLinkProposals`; `validateForensicProposals`; `validateEntityOps`; `importAuditJson`'s re-hash + schema validation), every mutation still a human Accept. **B8** row extended to the five. The scope key is derived PAGE-side (an id plus a SHA-256 of the exact request) and the receiver does not re-derive it: it is a dedupe key within one workspace, not an authorization, and a wrong key can only pair two of the operator's own requests in the workspace they share. New worker property closed in the same PR: `runEntityAuditPass` and `runForensicCorpusPass` had no abort (the 5-minute kill was their bound); a job heartbeats its worker alive, so both now abort at `timeoutForBudget` of their caps — no job holds the worker past its call. Still held-open, named in a guard: `xray:audit:module`, `xray:lens:read`, `xray:vision:describe`, `xray:llm:extract` (one unit each). No new network destination; no wire-format change. Guards: `tests/llm-jobs.test.mjs` (pass table = allowlist; every client pass a job or a named held-open exception; no retired type sent or handled; every consumer registered and flagging dropped channels), `tests/llm-job-consumers.test.mjs` (each consumer through the real runner), `tests/structure-guards.test.mjs` rule 4 (the closed registry). |
