...
| B6 | imported file → storage | backup / case bundle / entity sync | shape validation; `local_keys` **never** accrues on merge (`MERGE_EXCLUDED_KEYS`); import verifies rather than trusts. Keys enter the keystore only through `LocalKeyManager`'s locked writes: a bundle key through `importKey` (refuses an occupied name holding different material), a pulled sync key through `upsertKeys` (replaces its own name only; pubkey/npub/nsec derived from the private key, never taken from the payload; a non-hex key is counted malformed), and the pull's entity rows merge into a fresh registry read |
...
- **No keystore write erases what it did not touch.** Every
  `local_keys` write is a read-modify-write of FRESH storage under the
  `xray.local_keys` Web Lock that applies only its own names; a write
  never dumps a page's in-memory copy. Replacing the key under an
  occupied name is only through `upsertKeys` (the entity-sync pull and
  the side panel's `xray:user` reinstall). Guard-tested
...
- **G10 — every tab's content script holds the entity keystore.**
  `src/content/index.js` runs `LocalKeyManager.init()` on `<all_urls>`,
  loading every `local_keys` private key into the isolated world of
  each tab, and since 2026-09-24 re-reading it when it changes. Nothing
  in the content bundle reads that Map (verified 2026-09-24 against the
  bundle's inputs; the content script never writes keys). Page script
  cannot reach an isolated world, so the residual risk is a compromised
  renderer: the content script shares its process with the hostile page.
  The same init also reads `local_primary_identity` when signing is
  Local, but only into a local variable, to report signing state.
  *Proposed, not scheduled: drop the unused init.*
...
| 2026-09-24 | Keystore write path (JOURNAL 2026-09-24). No new boundary, message or destination; asset 1's handling changed. Every `local_keys` write is now a read-modify-write of fresh storage under the Web Lock `xray.local_keys`, re-checking the workspace pointer before it writes; the whole-Map `save()` is removed, because it let one page erase another page's keys and copy one workspace's keys into another. The entity-sync pull (B6) installs keys through `upsertKeys`, which replaces its own names only and derives pubkey/npub/nsec from the private key instead of trusting the payload, and it merges entity rows into a fresh read. Every page that runs `init()` now re-reads `local_keys` on each storage change. That includes the content script on every tab, which is new gap **G10** (pre-existing exposure, proposed fix: drop that init). New standing invariant: no keystore write erases what it did not touch. `local_keys` shape unchanged; no wire-format change. Guard: `tests/local-key-store.test.mjs`. |
