...
| 2026-09-25 | Entity-registry write path (JOURNAL 2026-09-25). No new boundary, message or destination. **B6**'s control is tightened: every `entities` write — bundle import, sync pull, backup restore / merge, and every local edit — is a read-modify-write of a fresh STRICT read under the Web Lock `xray.entities`, with the keystore's extension-origin refusal. A failed registry, storage-area or workspace-pointer read now writes nothing. Before this, a failed read was taken as an empty registry (erasing every other record, and letting a staler pulled key replace a local one) or as the default workspace (a reset cleared it; restores, merges and entity writes landed in it). For every other workspace content key, a plain read under an unreadable pointer reads nothing — never another workspace's data — and a plain write of that key refuses until a plain read of it succeeds, so no plain read-then-write carries one workspace's records into another. Wholesale writers nest the registry lock inside the keystore lock, never the reverse. `entities` shape unchanged; no wire-format change. Guard: `tests/entity-registry-store.test.mjs`. |
