...
| 2026-09-25 | Entity-registry write path (JOURNAL 2026-09-25). No new boundary, message or destination. **B6**'s control is tightened: every `entities` write — bundle import, sync pull, backup restore / merge, and every local edit — is a read-modify-write of a fresh STRICT read under the Web Lock `xray.entities`, with the keystore's extension-origin refusal. A failed registry, storage-area or workspace-pointer read now writes nothing. Before this, a failed read was taken as an empty registry (erasing every other record, and letting a staler pulled key replace a local one) or as the default workspace (a reset cleared it; restores, merges and entity writes landed in it). Every other workspace content key keeps its plain rule: an unreadable pointer reads as the default workspace and the page caches that, so its plain reads and writes stay in one workspace, as before (under the fault a plain write is misrouted into the default, where its read went). The strict paths (the locked writes, backup restore / merge / export, the reset and its safety backup, workspace removal) re-read the pointer, never use that cache, and write the workspace their read resolved, so a page whose cache fell back still writes, resets and removes in the ACTIVE workspace and cannot remove it. Asset 1: the keystore Map is loaded strictly too, so under a fallen-back cache it can belong to another workspace than the entity list; an entity record read through the plain cache joins a key, and `signEvent` signs, only while the Map's workspace equals the plain pointer's (`getKeyIn`) — otherwise the record reads keyless and signing refuses, so no display, kind-0 or mention signature, entity-sync push, or case bundle (which ships private keys) pairs one workspace's record with another workspace's key. Backup restore, reset and removal nest the registry lock inside the keystore lock; the merge takes only the registry lock; nothing takes them in the other order. `entities` shape unchanged; no wire-format change. Guard: `tests/entity-registry-store.test.mjs`. |
